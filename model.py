    ...
    def forward(self, input, attn_mask):
        ...
        with autocast(enabled=False):
            scores = torch.matmul(q_s.float(), k_s.float().transpose(-1, -2)) / (
                        self.output_dim ** 0.5)  # scores : [batch_size x n_heads x num_agents x num_agents]
...
class Network(nn.Module):
    ...
    def step(self, obs, pos):
        ...
        # def get_comm_mask(pos):
        # num_agents = obs.size(0)
        # mask = np.zeros((1, num_agents, num_agents), dtype=np.float32)
        # all_position = np.array(pos)
        # for i in range(num_agents):
        # self_position = all_position[i]
        # new_xy_array = np.array([self_position] * int(num_agents))
        # dist = torch.sqrt(np.sum(np.square(new_xy_array - all_position), axis=-1))
        # mask[:, i] = dist <= 5
        # return mask
        ...
        in_obs_mask = (pos_mat <= 5).all(2)
        # mask out agents that are far away
        # _, ranking = dist_mat.topk(min(self.max_comm_agents, num_agents), dim=1, largest=False)#返回了距离较小的3个agents的序号
        # dist_mask = torch.zeros((num_agents, num_agents), dtype=torch.bool)
        # dist_mask.scatter_(1, ranking, True)

        comm_mask = in_obs_mask
