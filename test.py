...
def test_model(model_range: Union[int, tuple]):
    ...
    if isinstance(model_range, int):
        state_dict = torch.load('./models/600000.pth', map_location=device)
        network.load_state_dict(state_dict)
        network.eval()
        ...
        for case in test_set:
            ...
            print("stay_on_goal: {}".format(stay))
            print("num_collide: {}".format(collide_1))
...
def test_one_case(args):
    ...
    (obs, pos), _, done, _ ,goal,collide= env.step(actions)

    length += 1
    max_goal = goal
    collide_num = collide
    return np.array_equal(env.agents_pos, env.goals_pos), step, max_goal, collide_num
